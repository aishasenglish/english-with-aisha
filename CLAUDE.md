...
- `content/batches.ts`, `content/testimonials.ts`, `content/faqs.ts`, `content/navigation.ts`
...
**Design tokens** are defined once in `app/globals.css` via Tailwind v4's `@theme` (`--color-ink`, `--color-teal`, `--color-coral`, `--color-ivory`, `--color-charcoal`, `--color-muted`, `--color-stone`, etc., plus `--font-sans`/`--font-serif` mapped to the Hanken Grotesk / Playfair Display fonts loaded in `app/layout.tsx`). Use these theme color/font names in Tailwind classes rather than raw hex values or arbitrary fonts.
