...
// Server component, no interaction required. Replaces the removed generic <IncludedList> on this
// page (IELTS Step 5). Two clearly separate lists side by side at lg+: the left column is the
// stable, already-approved teaching method (Steps 2-4) with no icon at all -- positive but
// non-promissory, never dressed up as a confirmed inclusion; the right column is a checklist of
// operational questions to confirm before paying, using a hollow "to confirm" marker rather than
// a checkmark so the two lists can never be mistaken for the same kind of claim.
export default function IELTSLearningFormat() {
  const { delivery } = ieltsPage;

  return (
    <section
      id={delivery.id}
      className="py-14 sm:py-16 px-4 bg-white"
      aria-labelledby="ielts-learning-format-heading"
    >
      <div className="max-w-5xl mx-auto">
        <div className="max-w-2xl mb-8 sm:mb-10">
          ...
          <h2 id="ielts-learning-format-heading" className="font-serif text-2xl sm:text-3xl font-medium text-ink mb-3">
            {delivery.heading}
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          {/* Details to confirm -- questions to ask, never inclusions */}
          <div>
              ...
              {delivery.detailsToConfirm.map((detail) => (
                <li key={detail.id} className="flex items-start gap-2.5 text-sm text-ink-soft leading-relaxed">
                  {/* Hollow circle, not a checkmark -- this is a question to confirm, not a
                      confirmed inclusion. Deliberately a different icon from every checkmark
                      used elsewhere on this page for support items and completed steps. */}
