 ...
 *  `schedule` to be omitted, but PTE Step 7 requires both before a record can render as a fully
...
type CompletePteIntake = Batch & { duration: string; schedule: string };

function isCompletePteIntake(batch: Batch): batch is CompletePteIntake {
 ...
 * which applies this rule to PTE specialist availability as well as IELTS.
 */
function displayStatus(batch: CompletePteIntake): Exclude<BatchStatus, "Closed"> {
...
function intakeEnquiryMessage(formattedDate: string, schedule: string): string {
  return `Hi Aisha! I am interested in the PTE intake starting ${formattedDate} at ${schedule} PKT. I need [PTE Academic or PTE Academic UKVI], my required overall and skill scores are [scores], my previous result is [details], my deadline is [date], and my country/time zone is [details]. Please confirm current availability, the complete fee and enrolment terms.`;
}

// Server component, no interaction required. Replaces the PTE Step 1 page-level heading +
// <BatchTable> wrapper (PTE Step 7) with a component that controls its own PTE-specific heading,
// no-intake copy and enquiry message rather than the shared cross-programme fallback.
export default function PTEAvailability() {
  const { availability } = ptePage;
  const completeIntakes = getPublishedUpcomingBatches("pte").filter(isCompletePteIntake);
  ...
  if (shown.length === 0) {
    return (
      <section
        ...
        aria-labelledby="pte-availability-heading"
      >
        <div className="max-w-2xl mx-auto">
          ...
          <h2 id="pte-availability-heading" className="font-serif text-2xl sm:text-3xl font-medium text-ink mb-3">
          ...
          {shown.map((batch) => {
            ...
            return (
              <article key={batch.id} className="border border-stone rounded-md bg-white p-5 sm:p-6">
                <h3 className="font-serif text-lg font-medium text-ink mb-1">PTE Academic Preparation</h3>
