      ...
      <div className="max-w-5xl mx-auto px-4 relative z-10 -mt-16 sm:-mt-20" id="finder">
        ...
        >
          <div className="flex flex-wrap items-baseline justify-between gap-2 mb-5">
            <h2 className="font-serif text-xl sm:text-2xl font-bold text-ink">Find your course</h2>
            <span className="text-sm text-muted">Filters the course list below.</span>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            <label className="flex flex-col gap-1.5">
              <span className="font-serif text-xs font-bold uppercase tracking-widest text-muted">
                What do you want to achieve?
              </span>
              <select
                value={goal}
                onChange={(e) => setGoal(e.target.value as Goal | "all")}
                className="w-full px-4 py-3.5 rounded-[2px] border border-line-strong bg-white text-ink text-sm focus:outline-none focus:ring-2 focus:ring-coral cursor-pointer"
              >
            ...
            <label className="flex flex-col gap-1.5">
              <span className="font-serif text-xs font-bold uppercase tracking-widest text-muted">Course</span>
              <select
                value={slug}
            ...
            <label className="flex flex-col gap-1.5">
              <span className="font-serif text-xs font-bold uppercase tracking-widest text-muted">Format</span>
              <select
                value={format}
      ...
      <section className="pt-16 sm:pt-20 pb-20 px-4" id="courses">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-wrap items-end justify-between gap-4 mb-10">
            ...
            <Link
              href="/courses"
              className="font-serif text-sm font-bold uppercase tracking-wide border-b-2 border-coral pb-1 hover:text-amber-dark inline-flex items-center gap-2"
            >
              All courses
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {visible.map((c) => (
              <Link
                href={`/courses/${c.slug}`}
                key={c.slug}
                className="group bg-white border border-stone hover:border-line-strong hover:-translate-y-1 hover:shadow-xl transition-all duration-200 flex flex-col"
              >
          ...
          {visible.length === 0 && (
            <p className="mt-8 p-6 bg-ivory border-l-[3px] border-coral text-charcoal">
              ...
              <a href={whatsappLink()} className="text-amber-dark font-semibold">
                Message Aisha
              </a>{" "}
