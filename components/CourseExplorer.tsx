          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {visible.map((c) => (
              <Link
                href={`/courses/${c.slug}`}
                key={c.slug}
                className="rounded-md border border-stone hover:border-line-strong hover:-translate-y-1 transition-[transform,border-color] duration-200 flex flex-col p-8 bg-[rgba(240,240,240,0.4)]"
              >
                {c.badge && (
                  <span
