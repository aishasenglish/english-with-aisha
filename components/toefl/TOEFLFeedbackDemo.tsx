...
export default function TOEFLFeedbackDemo() {
  const { feedbackDemo } = toeflPage;

  return (
    <section
      id={feedbackDemo.id}
      className="py-14 sm:py-16 px-4 bg-ivory"
      aria-labelledby="toefl-feedback-example-heading"
    >
      <div className="max-w-3xl mx-auto">
        <header className="mb-6">
          ...
          <h2 id="toefl-feedback-example-heading" className="font-serif text-2xl sm:text-3xl font-medium text-ink mb-3">
        ...
        <div className="space-y-6">
          <article aria-labelledby="toefl-situation-heading">
            <p className="text-xs font-medium uppercase tracking-wide text-ink-faint mb-2">
              {feedbackDemo.taskLabel}
            </p>
            <h3 id="toefl-situation-heading" className="font-serif text-base font-medium text-ink mb-2">
          ...
          <article>
            ...
            {/* No quotation marks — this is an illustrative message written for the site, not a
                quotation attributed to a real learner. */}
          ...
          <article>
            ...
            <div className="bg-green-50 border border-green-200 rounded-md p-4 sm:p-5 text-charcoal leading-relaxed">
              <p className="font-medium mb-3">{feedbackDemo.revisedAttemptSubject}</p>
              {feedbackDemo.revisedAttemptParagraphs.map((paragraph, i) => (
          ...
          <p className="text-sm text-ink-soft leading-relaxed border-t border-stone pt-5">
            {feedbackDemo.scoringBoundary}
          </p>
          <p className="text-sm text-ink-soft leading-relaxed">{feedbackDemo.speakingBoundary}</p>
        </div>
      </div>
    </section>
  );
