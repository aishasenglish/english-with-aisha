...
import { spokenEnglishPricing, isValidPublishedSpokenEnglishPrice } from "@/content/spokenEnglishPricing";

// Server component, no interaction required. Branches explicitly on validated pricing status --
// this is the ONLY place on the site allowed to render an exact Spoken English fee, and only once
// BOTH isValidPublishedSpokenEnglishPrice() confirms the record is complete, well-formed and
...
// `site.showPrices` alone) -- see docs/spoken-english-offer-verification.md's pricing section.
// Any malformed or expired "published" record fails safely back to the enquiry panel rather than
// rendering a partial or stale price. Never imports the generic PricingCard or
// content/courses.ts's legacy spoken-english `price` field. Mirrors
// components/toefl/TOEFLPricing.tsx and components/pte/PTEPricing.tsx (this file additionally
// gates on `site.showPrices`, which those two do not yet check).
export default function SpokenEnglishPricing() {
  const { pricing } = spokenEnglishPage;

  // Written as a single inline condition (not a separately-stored `mayPublish` boolean) so
  // TypeScript's control-flow narrowing still recognises `spokenEnglishPricing` as the validated
  ...
  if (!site.showPrices || !isValidPublishedSpokenEnglishPrice(spokenEnglishPricing)) {
    const { enquire } = pricing;
    return (
      <section
        id={pricing.id}
        className="py-14 sm:py-16 px-4 bg-ivory"
        aria-labelledby="spoken-english-pricing-heading"
      >
        <div className="max-w-2xl mx-auto">
          <div className="border border-stone rounded-md bg-white p-6 sm:p-8">
            ...
            <h2 id="spoken-english-pricing-heading" className="font-serif text-2xl sm:text-3xl font-medium text-ink mb-3">
              {enquire.heading}
            </h2>
  ...
  // currently makes this branch render -- content/spokenEnglishPricing.ts's
  // spokenEnglishPricing.status is "enquire" until Aisha approves a real fee.
  const { published } = pricing;
  const record = spokenEnglishPricing;
  const includedItems = spokenEnglishPage.delivery.approachItems.filter((item) =>
  ...
  const enquiryMessage = `Hi Aisha! I am interested in Spoken English coaching (${record.optionLabel}, ${record.formatLabel}, ${formattedAmount} ${record.billingBasis}). Please confirm whether this option is currently available and the next steps.`;
    ...
    >
      <div className="max-w-2xl mx-auto">
        <div className="border border-stone rounded-md bg-white p-6 sm:p-8">
          ...
          <h2 id="spoken-english-pricing-heading" className="font-serif text-2xl sm:text-3xl font-medium text-ink mb-1">
            {published.heading}
          </h2>
