...
// app/courses/english-writing/page.tsx -- both read from the same content/englishWriting.ts
// `breadcrumb` array so the visible path and the structured data can never disagree. Placed above
// the hero rather than inside it, so it doesn't compete with the H1 for attention. Mirrors
// components/spoken-english/SpokenEnglishBreadcrumb.tsx / components/toefl/TOEFLBreadcrumb.tsx /
// components/pte/PTEBreadcrumb.tsx / components/ielts/IELTSBreadcrumb.tsx.
export default function EnglishWritingBreadcrumb() {
  const { breadcrumb } = englishWritingContent;
