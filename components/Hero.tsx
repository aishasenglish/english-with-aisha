    ...
    <section className="relative bg-ink text-white overflow-hidden">
      <div className="absolute inset-0">
        <Image
          ...
          priority
          sizes="100vw"
          className="object-cover object-[60%_22%] opacity-55"
        />
      </div>
      <div
        ...
        style={{
          background:
            "linear-gradient(100deg, rgba(21,24,28,.96) 0%, rgba(21,24,28,.86) 38%, rgba(21,24,28,.4) 72%, rgba(21,24,28,.2) 100%)",
        }}
        aria-hidden
      />
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 sm:pt-24 pb-32 sm:pb-40">
        <div className="max-w-2xl">
          <p className="font-serif text-xs font-bold uppercase tracking-widest text-coral flex items-center gap-3 mb-5">
            {site.credentials}
          </p>
          ...
          <div className="flex flex-wrap gap-x-9 gap-y-3 pt-7 border-t border-white/15">
            {FACTS.map((f) => (
              <span key={f} className="flex items-center gap-2.5 text-sm text-white/75">
                <TickIcon />
                {f}
              </span>
            ))}
